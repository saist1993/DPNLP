...
def train(model, iterator, optimizer, criterion, device, accuracy_calculation_function, other_params):
    ...
    for items in tqdm(iterator):
        ...
        if is_regression:
            ...
            if is_adv:
                loss_aux_1 = criterion(output['adv_output_1'].squeeze(), items['aux'].squeeze())
                loss_aux_2 = criterion(output['adv_output_2'].squeeze(), items['aux'].squeeze())
                loss_aux_3 = criterion(output['adv_output_3'].squeeze(), items['aux'].squeeze())
                loss_aux_1_2 = diff_loss.forward(output['adv_other_layer_output_1'][-2], output['adv_other_layer_output_2'][-2])
                loss_aux_1_3 = diff_loss.forward(output['adv_other_layer_output_1'][-2], output['adv_other_layer_output_3'][-2])
        else:
    ...
    with torch.no_grad():
        for items in tqdm(iterator):
            ...
            if is_regression:
                ...
                loss_main = criterion(output['prediction'], items['labels'])
                if is_adv:
                    loss_aux = criterion(output['adv_output_1'], items['aux'])
